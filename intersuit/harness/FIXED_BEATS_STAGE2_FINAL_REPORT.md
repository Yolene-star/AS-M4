...
Fixed Window BEATs 基线已完成 Stage 2 正式训练并通过本轮小规模效果门禁。
...
训练时固定 gate 为 `1.0`，保证 Projector/Fusion 获得足够梯度；推理时使用独立校准 gate `0.25`，防止音频残差过大导致生成退化。原始音频窗口被判定为静音时，固定链路会把对应 gate 硬置零。

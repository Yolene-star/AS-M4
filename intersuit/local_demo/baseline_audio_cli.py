...
def save_chattts_audio(text, audio_path):
    from scipy.io import wavfile

    wav = chat.infer(clean_text(text))
    if isinstance(wav, list):
        wav = wav[0]
    wav = np.asarray(wav)
    if wav.ndim > 1:
        wav = np.squeeze(wav)
    wavfile.write(audio_path, 24000, wav.astype(np.float32))
...
class InterSuit:
    ...
    def output(self, requests: dict, gen_kwargs: dict):
        ...
        if requests["question_audio"] is not None:
            ...
            audio_path = "./local_demo/wav/tts_" + visuals[0].split("/")[-1]+".wav"
            if os.path.exists(audio_path): os.remove(audio_path) # refresh
            if not os.path.exists(audio_path):
                save_chattts_audio(context, audio_path)
...
if __name__ == "__main__":
    ...
    model = InterSuit(pretrained="checkpoints/LongVA-7B-Qwen2-Audio", model_name="llava_qwen", device_map=args.device, attn_implementation="eager")

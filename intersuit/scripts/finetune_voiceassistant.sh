...
LLM_VERSION="${LLM_VERSION:-checkpoints/LongVA-7B-Qwen2}"
...
MID_RUN_NAME="${MID_RUN_NAME:-LongVA-7B-Qwen2-VoiceAssistant-400K-all}"
echo "MID_RUN_NAME: ${MID_RUN_NAME}"


CKPT_PATH="${CKPT_PATH:-$LLM_VERSION}" # this could also be the previous stage checkpoint
DEEPSPEED_CONFIG="${DEEPSPEED_CONFIG:-scripts/zero2_offload.json}"
DATA_PATH="${DATA_PATH:-inputs/texts/VoiceAssistant.json}"
IMAGE_FOLDER="${IMAGE_FOLDER:-inputs/images/llava-next}"
SPEECH_FOLDER="${SPEECH_FOLDER:-inputs/speech/VoiceAssistant}"

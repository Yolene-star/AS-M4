...
class ModelArguments:
    ...
    enable_scene_audio: bool = field(default=False)
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    speech_normalize: Optional[bool] = False
    speech_projector_lr: Optional[float] = None
    scene_audio_projector_lr: Optional[float] = None
    streaming_av_lr: Optional[float] = None
...
def find_all_linear_names(model):
    ...
    multimodal_keywords = ["mm_projector", "vision_tower", "vision_resampler", "speech_projector"]
...
def preprocess_multimodal(sources: Sequence[str], data_args: DataArguments) -> Dict:
    ...
    for source in sources:
        for sentence in source:
            ...
            # NOTE --> keep interleave data, this is vulnerable to data, don't know what will happen
            if "mmtag" in conversation_lib.default_conversation.version:
                sentence["value"] = sentence["value"].replace(DEFAULT_IMAGE_TOKEN, "<Image>" + DEFAULT_IMAGE_TOKEN + "</Image>")
...
def preprocess_multimodal_av(sources: Sequence[str], data_args: DataArguments) -> Dict:
    is_multimodal = data_args.is_multimodal
    if not is_multimodal:
        return sources
    ...
    for source in sources:
        for sentence in source:
            ...
            if data_args.mm_use_im_start_end:
                replace_token = DEFAULT_IM_START_TOKEN + replace_token + DEFAULT_IM_END_TOKEN
            sentence["value"] = sentence["value"].replace(DEFAULT_IMAGE_TOKEN, replace_token)
            replace_token = DEFAULT_SPEECH_TOKEN
            sentence["value"] = sentence["value"].replace(DEFAULT_SPEECH_TOKEN, replace_token)
...
def preprocess_qwen(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, max_len=2048, system_message: str = "You are a helpful assistant.") -> Dict:
    ...
    # add <image> temporarily for preprocessing
    ...
    for i, source in enumerate(sources):
        ...
        for j, sentence in enumerate(source):
            ...
            if has_image and "<image>" in sentence["value"]:
                ...
                # NOTE allow insert <image> token
                _input_id = tokenizer(role).input_ids + nl_tokens + tokenizer(sentence["value"]).input_ids + [im_end] + nl_tokens
            elif has_image and "<video>" in sentence["value"]:
                # # TODO maybe this should be changed for interleaved data?
                # num_im = len(re.findall(DEFAULT_IMAGE_TOKEN, sentence["value"]))
                # # multi image can start with text first
                # if num_im == 1:
                #     assert sentence["value"].startswith("<video>"), print(sentence["value"])
                # matches = re.findall("<video>", sentence["value"])
                # num_image = len(matches)
                # _input_id = tokenizer(role).input_ids + nl_tokens + [IMAGE_TOKEN_INDEX] * num_image + nl_tokens + tokenizer(sentence["value"][len("<image>") * num_image :]).input_ids + [im_end] + nl_tokens
                # NOTE allow insert <image> token
                _input_id = tokenizer(role).input_ids + nl_tokens + tokenizer(sentence["value"]).input_ids + [im_end] + nl_tokens
            ...
            target += _target
        assert len(input_id) == len(target)
        # recover special visual tokens
        for idx, encode_id in enumerate(input_id):
            if encode_id == special_image_token_index or encode_id == special_video_token_index:
...
def preprocess_qwen_av(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, max_len=2048, system_message: str = "You are a helpful assistant.") -> Dict:
    ...
    special_image_token_index = tokenizer.convert_tokens_to_ids("<image>")
    special_video_token_index = tokenizer.convert_tokens_to_ids("<image>")
    special_speech_token_index = tokenizer.convert_tokens_to_ids("<speech>")
    ...
    for i, source in enumerate(sources):
        ...
        for idx, encode_id in enumerate(input_id):
            if encode_id == special_image_token_index or encode_id == special_video_token_index:
                input_id[idx] = IMAGE_TOKEN_INDEX
            if encode_id == special_speech_token_index:
...
class LazySupervisedDataset(Dataset):
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "speech" in sources[0]:
            ...
            speech_dir = self.data_args.speech_folder
            if type(speech_file) is list:
                speech = []
                for s in speech_file:
                    s = os.path.join(speech_dir, s)
                    speech_tensor = process_audio_with_whisper(s)
        ...
        if "image" in sources[0]:
            ...
            if "video_features" in sources[0]:
                video_features = _load_feature_tensor(sources[0]["video_features"], self.data_args.video_feature_folder)
                image = [(video_features, (0, 0), "video_feature")]
                sources = preprocess_multimodal(copy.deepcopy([e["conversations"] for e in sources]), self.data_args)
            elif 'speech' in sources[0]:
                sources = preprocess_multimodal_av(copy.deepcopy([e["conversations"] for e in sources]), self.data_args)
            else:
    ...
    def _load_scene_audio_fields(self, sample: Dict) -> Dict[str, torch.Tensor]:
        """Load pre-windowed or precomputed scene-audio fields.

        This does not touch the existing ``speech`` path. First-pass AS-M4
        training should prefer precomputed scene-audio tensors to keep 32k runs
        memory-stable and reproducible.
        """
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if any("scene_audio" in instance for instance in instances):
            scene_audio_tensors = [instance.get("scene_audio") for instance in instances]
            batch["scene_audios"], batch["scene_audio_mask"] = _pad_scene_audio_batch(scene_audio_tensors)
            timestamp_tensors = [
                instance.get("scene_audio_timestamps")
                for instance in instances
            ]
            batch["scene_audio_timestamps"] = _pad_scene_audio_timestamps(
                timestamp_tensors,
                batch["scene_audios"].shape[1],
                batch["scene_audios"].device,
            )
            if any("frame_timestamps" in instance for instance in instances):
                batch["frame_timestamps"] = [
                    instance.get("frame_timestamps") for instance in instances
                ]
            for key in ("event_labels", "offset_labels", "gate_labels", "pseudo_confidence"):
...
def _scene_audio_to_tensor(value, folder: Optional[str] = None) -> torch.Tensor:
    tensor = _load_feature_tensor(value, folder)
    if tensor.ndim == 1:
...
def _load_feature_tensor(value, folder: Optional[str] = None) -> torch.Tensor:
    if isinstance(value, torch.Tensor):
        ...
        path = value if folder is None else os.path.join(folder, value)
        tensor = torch.load(path, map_location="cpu")
        if isinstance(tensor, dict):
            tensor = tensor.get("features", tensor.get("scene_audio", tensor.get("video_features", tensor.get("audio"))))
            if tensor is None:
                raise ValueError(f"Feature tensor dict from {path} lacks features/scene_audio/video_features/audio")
    else:
        tensor = torch.as_tensor(value)
    return tensor.to(dtype=torch.float32)


def _pad_scene_audio_batch(scene_audio_tensors):
    present = [tensor for tensor in scene_audio_tensors if tensor is not None]
    if not present:
        return torch.empty(len(scene_audio_tensors), 0, 0), torch.zeros(len(scene_audio_tensors), 0, dtype=torch.bool)
    feature_dim = present[0].shape[-1]
    dtype = present[0].dtype
    max_steps = max(tensor.shape[0] if tensor is not None else 0 for tensor in scene_audio_tensors)
    batch = torch.zeros(len(scene_audio_tensors), max_steps, feature_dim, dtype=dtype)
    mask = torch.zeros(len(scene_audio_tensors), max_steps, dtype=torch.bool)
    for idx, tensor in enumerate(scene_audio_tensors):
        if tensor is None:
            continue
        if tensor.shape[-1] != feature_dim:
            raise ValueError("All scene_audio tensors in a batch must share feature dimension")
        steps = tensor.shape[0]
        batch[idx, :steps] = tensor
        mask[idx, :steps] = True
    return batch, mask


def _pad_scene_audio_timestamps(timestamp_tensors, max_steps: int, device: torch.device):
    batch = torch.zeros(len(timestamp_tensors), max_steps, 2, dtype=torch.float32, device=device)
    for idx, timestamps in enumerate(timestamp_tensors):
        if timestamps is None:
            continue
        ts = torch.as_tensor(timestamps, dtype=torch.float32, device=device)
        if ts.ndim != 2 or ts.shape[-1] != 2:
            raise ValueError(f"scene_audio_timestamps must be shaped [T,2], got {tuple(ts.shape)}")
        steps = min(ts.shape[0], max_steps)
        batch[idx, :steps] = ts[:steps]
    return batch
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        if model_args.speech_encoder is not None:
            model.get_model().initialize_speech_modules(model_args=model_args, fsdp=training_args.fsdp)
        ...
        if model_args.enable_scene_audio:
            if model_args.scene_audio_hidden_size is None:
                model_args.scene_audio_hidden_size = model.config.hidden_size
            if model_args.streaming_av_align_dim is None:
                model_args.streaming_av_align_dim = model.config.hidden_size
            model.get_model().initialize_scene_audio_modules(model_args=model_args, fsdp=training_args.fsdp)
            scene_audio_encoder = model.get_scene_audio_encoder()
            if scene_audio_encoder is not None:
                scene_audio_encoder.to(dtype=torch.bfloat16 if training_args.bf16 else torch.float16, device=training_args.device)
            streaming_av_module = model.get_streaming_av_module()
            if streaming_av_module is not None:
                streaming_av_module.to(dtype=torch.bfloat16 if training_args.bf16 else torch.float16, device=training_args.device)
        ...
        if model_args.mm_tunable_parts is None:  # traditional way of deciding which part to train
            ...
            if model_args.enable_scene_audio:
                if model.get_scene_audio_encoder() is not None:
                    model.get_scene_audio_encoder().requires_grad_(False)
                if model.get_streaming_av_module() is not None:
                    model.get_streaming_av_module().requires_grad_(False)
            ...
            if "speech_encoder" in tunable_parts:
                for name, param in model.named_parameters():
            ...
            if "mm_language_model" in tunable_parts:
                for name, param in model.named_parameters():
                    if "vision_tower" not in name and "mm_projector" not in name and "vision_resampler" not in name and "speech_encoder" not in name and "speech_projector" not in name and "scene_audio_encoder" not in name and "streaming_av_module" not in name:
                        param.requires_grad_(True)

        freeze_lm_head = os.environ.get("M4_FREEZE_LM_HEAD", "0").lower() in {"1", "true", "yes", "on"}
        if freeze_lm_head and hasattr(model, "lm_head"):
            for p in model.lm_head.parameters():
                p.requires_grad_(False)
            rank0_print("M4_FREEZE_LM_HEAD=1: 已冻结 lm_head，避免 ZeRO-3 在 lm_head 反向传播时产生约 1GB 的梯度峰值。")
        ...
        if model_args.speech_encoder is not None:
            model.config.speech_normalize = training_args.speech_normalize
            model.config.speech_projector_lr = training_args.speech_projector_lr
        if model_args.enable_scene_audio:
            model.config.scene_audio_projector_lr = training_args.scene_audio_projector_lr
            model.config.streaming_av_lr = training_args.streaming_av_lr

...
from intersuit.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, SPEECH_TOKEN_INDEX
...
class LlavaQwenForCausalLM(Qwen2ForCausalLM, LlavaMetaForCausalLM):
    ...
    def _use_chunked_lm_loss(self, labels: Optional[torch.LongTensor]) -> bool:
        if labels is None or not self.training:
            return False
        enabled = os.environ.get("M4_CHUNKED_LM_LOSS", "0").lower()
        return enabled in {"1", "true", "yes", "on"}

    def _chunked_lm_loss(self, hidden_states: torch.Tensor, labels: torch.LongTensor) -> torch.Tensor:
        chunk_size = int(os.environ.get("M4_CHUNKED_LM_LOSS_TOKENS", "512"))
        chunk_size = max(chunk_size, 1)
        checkpoint_loss = os.environ.get("M4_CHUNKED_LM_LOSS_CHECKPOINT", "1").lower() in {"1", "true", "yes", "on"}

        shift_hidden_states = hidden_states[..., :-1, :].contiguous()
        shift_labels = labels[..., 1:].contiguous().to(shift_hidden_states.device)
        flat_hidden_states = shift_hidden_states.view(-1, shift_hidden_states.size(-1))
        flat_labels = shift_labels.view(-1)
        local_chunks = (flat_labels.numel() + chunk_size - 1) // chunk_size

        if torch.distributed.is_available() and torch.distributed.is_initialized():
            max_chunks = torch.tensor(local_chunks, device=shift_hidden_states.device, dtype=torch.long)
            torch.distributed.all_reduce(max_chunks, op=torch.distributed.ReduceOp.MAX)
            num_chunks = int(max_chunks.item())
        else:
            num_chunks = local_chunks

        loss_sum = flat_hidden_states.new_zeros((), dtype=torch.float32)
        valid_tokens = flat_labels.new_zeros((), dtype=torch.long)

        for chunk_idx in range(num_chunks):
            start = chunk_idx * chunk_size
            end = min(start + chunk_size, flat_labels.numel())
            if start < flat_labels.numel():
                chunk_hidden_states = flat_hidden_states[start:end]
                chunk_labels = flat_labels[start:end]
                if chunk_hidden_states.size(0) < chunk_size:
                    pad_tokens = chunk_size - chunk_hidden_states.size(0)
                    pad_hidden_states = flat_hidden_states[:1].expand(pad_tokens, -1)
                    pad_labels = chunk_labels.new_full((pad_tokens,), IGNORE_INDEX)
                    chunk_hidden_states = torch.cat([chunk_hidden_states, pad_hidden_states], dim=0)
                    chunk_labels = torch.cat([chunk_labels, pad_labels], dim=0)

                def chunk_forward(hidden, target):
                ...
                if checkpoint_loss and chunk_hidden_states.requires_grad:
                    chunk_loss = checkpoint(chunk_forward, chunk_hidden_states, chunk_labels, use_reentrant=False)
                else:
                    chunk_loss = chunk_forward(chunk_hidden_states, chunk_labels)

                loss_sum = loss_sum + chunk_loss
                valid_tokens = valid_tokens + chunk_labels.ne(IGNORE_INDEX).sum()
            else:
                dummy_hidden_states = flat_hidden_states[:1].expand(chunk_size, -1)
                dummy_logits = self.lm_head(dummy_hidden_states)
                loss_sum = loss_sum + dummy_logits.float().sum() * 0.0
    ...
    def forward(
        ...
        elif self._use_chunked_lm_loss(labels):
    ...
    def generate_streaming(
        ...
        **kwargs,
        ...
        outputs_ids = torch.empty((inputs_embeds.shape[0], 0), dtype=torch.long, device=inputs_embeds.device)
        for idx in range(max_new_tokens):
            outputs = super().forward(
                past_key_values=past_key_values,
                inputs_embeds=inputs_embeds,
            ...
            if next_token_ids.numel() == 0:
                break
            if outputs_ids.shape[-1] == 0:
                outputs_ids = next_token_ids
            else:
                outputs_ids = torch.cat([outputs_ids, next_token_ids], dim=-1)
            inputs_embeds = self.get_model().embed_tokens(next_token_ids)

            if tokenizer is not None and next_token_ids.squeeze(0)[0].item() == tokenizer.eos_token_id:
                break
